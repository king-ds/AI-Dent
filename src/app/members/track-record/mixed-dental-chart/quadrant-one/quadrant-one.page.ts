...
export class QuadrantOnePage implements OnInit {
  ...
  ionViewWillEnter(){
    this.loader = true;
    let teethNumber = [11, 12, 13, 14, 15, 16, 17, 18, 51, 52, 53, 54, 55]

    // Check for the existing teeth number
    this.apiService.getDentalChartQ1(this.trackRecord['id']).subscribe((val : any) => {
  ...
  OnChange(event){
    ...
    if(selectedData.includes(6)){
      ...
      if(selectedData == 6){
        ...
        console.log("It is impossible to have restoration or caries if caries free is selected.");
