...
export class MedicalHealthQuestionnairePage implements OnInit {
  ...
  loader : boolean;
  isReadOnly = true;
  hasMedicalHeathQuestionnaire : boolean;
  medicalHeatlhQuestionnaireId : string;
  ...
  ionViewWillEnter(){
    ...
    this.debouncer = setTimeout(() => {
      this.apiService.getMedicalHealthQuestionnaire(this.track_record['id']).subscribe((val) => {
        if(val==null){
          this.hasMedicalHeathQuestionnaire = false;
        }else{
          this.hasMedicalHeathQuestionnaire = true;
  ...
  submitEditedMedicalHealthQuestionnaire(){
    ...
    if(this.hasMedicalHeathQuestionnaire){
      this.debouncer = setTimeout(() => {
      ...
      this.apiService.addMedicalHistoryQuestionnaire(medicalHealthQuestionnaireData).then(res => {
        this.medicalHeatlhQuestionnaireId = res['id']
        let trackRecordData = {
          "medical_health_questionnaire" : this.medicalHeatlhQuestionnaireId
        }
